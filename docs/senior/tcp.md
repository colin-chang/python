...
## 2. 四次挥手
![TCP三次握手四次挥手与十种状态](https://s2.ax1x.com/2020/01/20/1ivkLV.jpg)
...
![TCP通信模型](https://s2.ax1x.com/2020/01/20/1ivEZT.png)
...
```
![客户端共享TCP连接](https://s2.ax1x.com/2020/01/20/1ivFs0.jpg)
...
不同的开发平台中对关系型数据库和非关系型数据库都有其各自的对数据库连接池实现和应用。以.Net平台为例，访问关系型数据库完成后调用`IDbConnection`对象的`Dispose()`时(多通过`using`关键字自动实现)会释放`IDbConnection`对象并将其占用的数据库连接归还到连接池中。在非关系型数据库中也有对数据库连接池的应用，如[MongoDB连接池](https://architecture.ccstudio.org/nosql/mongo.html#_8-2-mongo-连接池),在`Mongo Driver`中提供的`MongoClient`对象本身就实现了一个线程安全的连接池。以下是来自其官方的阐述。
